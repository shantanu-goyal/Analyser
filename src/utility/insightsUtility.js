...
function getDirectThirdParty(thirdPartyData, requestInitiators, allScriptsSet) {
  ...
  directThirdParty.forEach((script) => {
    ...
    result.forEach((item) => {
      ...
      if (
        directThirdParty.indexOf(item) !== -1 &&
        (!adjList.get(item) || adjList.get(item).indexOf(script) === -1)
      ) {
        const index = directThirdParty.indexOf(item);
        directThirdParty.splice(index, 1);
        scriptMap.delete(item);
      }
    });

    if (result.length > 0) {
      let entityScripts = scriptMap.get(script) || [];
      entityScripts = [...entityScripts, ...result];
      scriptMap.set(script, entityScripts);
    }
  });
  const entityMap = new Map();
  scriptMap.forEach((value, key) => {
    const name = getEntityName(thirdPartyData, key);
    let entityScripts = entityMap.get(name) || [];
    entityScripts = [...entityScripts, ...value];
    entityMap.set(name, entityScripts);
  });
  return entityMap;
}
...
export function getEntityMappings(
  ...
  const entityMap = getDirectThirdParty(
    thirdPartyData,
    requestInitiators,
    allScriptsArray
  );
...
export function getThirdPartyDataWithNetworkDetails(
  ...
  if (thirdPartyData.length > 0) {
    thirdPartyWithNetwork = thirdPartyData.reduce((acc, item) => {
