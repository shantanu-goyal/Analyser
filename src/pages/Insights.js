...
export default function Insights(){
    ...
    function extractMainThreadWorkBreakDownData(data){
        ...
        for(let i=0;i<result.length;i++){
            if(result[i].groupLabel==="Script Evaluation"){
                result[i]={
                    ...result[i],
                    suggestions:["Optimize third-party JavaScript",
                ...
                result[i]={
                    ...result[i],
                    suggestions:["Reduce the scope and complexity of style calculations",
                ...
                result[i]={
                    ...result[i],
                    suggestions:["Stick to compositor only properties and manage layer count",
                ...
                result[i]={
                    ...result[i],
                    suggestions:["Reduce JavaScript payloads with code splitting",
                    "Remove unused code"
                ]
                }
            }
                ...
                result[i]={
                    ...result[i],
                    suggestions:["Extract critical CSS",
                    "Minify CSS",
                ...
                result[i]={
                    ...result[i],
                    suggestions:["Monitor your web page's total memory usage with measureMemory()"]
                }
            }
        }
        return result;
    ...
    function extractNetworkRoundTripTime(data){
        ...
        const finalResult={
            data:result,
            suggestions:['Try to use a CDN. CDN is able to optimize network pathways between locations, resulting in reduced RTT and improved latency for visitors accessing content cached inside the CDN.']
        }
        return finalResult;
    }
    ...
    function extractResourceSummaryData(data){
        ...
        const result=details.items.map(item=>{
            if(item.label==='Script'){
                return {
                    ...item,
                    suggestions:["Apply instant loading with the PRPL pattern",
    ...
    function extractThirdPartySummaryData(data){
        ...
        const finalResult={
            data:results,
            suggestions:[
                'Try to move third party scripts to PartyTown'
            ]
        }
        return finalResult;
    }

    console.log(extractThirdPartySummaryData(data));
    console.log(extractResourceSummaryData(data));
    console.log(extractNetworkServerLatencyData(data));
    console.log(extractNetworkRoundTripTime(data));
    console.log(extractBootupTimeData(data));
    console.log(extractMainThreadWorkBreakDownData(data));
    ...
    return(
        ...
        {data &&(
            <div>
                ...
                <h5 style={{ textAlign: "center" }}>Insights gathered from the analysis</h5>
            </div>
        )}
        </>
    )
